...
game.party.push(pokemon[40], pokemon[65], pokemon[2]);
...
game.catchPokemon = (pokemonObj) => {
  if(game.items[1].quantity < 1) {
    console.log('you have run out of pokeballs!');
  } else {
    game.items[1].quantity -= 1;
    if(game.partyCount < 6){
      game.party.push(pokemonObj);
...
game.catchPokemon = (pokemonName) => {
  let pokeNameFound = false;
  if(game.items[1].quantity < 1) {
    console.log('you have run out of pokeballs!');
  } else {
    pokemon.forEach((poke) => {
      if(poke.name.toLowerCase() === pokemonName.toLowerCase()) {
        game.items[1].quantity -= 1;
        pokeNameFound = true;
        if(game.partyCount < 6) {
...
sortedByType = pokemon.reduce((sorted, p) => {
  !sorted[p.type] ? sorted[p.type] = [p] : sorted[p.type].push(p);
  return sorted;
}, {});

// let newType = '';
// pokemon.forEach((poke) => {
//   if(!sortedByType.hasOwnProperty(poke.type)) {
//     newType = poke.type;
//     sortedByType[newType] = [];
//   } 
//   sortedByType[poke.type].push(poke);
// });
